// 移植自 wheeltec_ros2/src/turn_on_wheeltec_robot/include/turn_on_wheeltec_robot/wheeltec_robot.h (ROS2 Humble)
// Ported from wheeltec_robot.h (ROS2 Humble). Changes:
// 1. 头文件保护宏 __WHEELTEC_ROBOT_H_ -> __K7_ROBOT_H_
// 2. 类名 turn_on_robot -> K7SerialNode
// 3. 删除 wheeltec_robot_msg 依赖（K7 工作区无此消息包）：两个 msg 头文件 include、
//    distance_publisher 成员、Publish_distance() 声明（均为死代码，cpp 中从未定义/实例化/调用）
// 4. 新增 cmd_vel 看门狗成员：cmd_vel_timeout_ms、last_cmd_vel_time_、cmd_vel_watchdog_timer_，
//    以及 Send_Cmd_Vel()（组帧发送小私有函数，供 Cmd_Vel_Callback 与看门狗复用）、
//    Cmd_Vel_Watchdog_Check() 声明
// 其余（协议宏、协方差矩阵、结构体、成员变量）与 wheeltec 原文件一致

#ifndef __K7_ROBOT_H_
#define __K7_ROBOT_H_
...
class K7SerialNode : public rclcpp::Node
{
	public:
		K7SerialNode();  //Constructor //构造函数
		~K7SerialNode(); //Destructor //析构函数
	...
	private:
		...
		//cmd_vel 看门狗：K7 固件无命令丢失保护，断链后下位机会保持最后一次速度，必须主动发零速
		//cmd_vel watchdog: the K7 firmware keeps the last velocity on link loss, so zero speed must be sent actively
		void Send_Cmd_Vel(double linear_x, double linear_y, double angular_z); //Frame and send the 11-byte velocity command //组帧并发送11字节速度指令
		void Cmd_Vel_Watchdog_Check(); //Watchdog check, send zero speed on timeout //看门狗检查，超时发零速
		rclcpp::TimerBase::SharedPtr cmd_vel_watchdog_timer_; //100ms period wall timer //100ms周期看门狗定时器
		rclcpp::Time last_cmd_vel_time_; //Time of the latest received cmd_vel //最近一次收到cmd_vel的时间
