...
//S21C数据
static const uint8_t s21c_packetLen = sizeof(s21c_board);                           //数据长度
static const uint8_t s21c_packetNum = (s21c_packetLen/7) + ((s21c_packetLen%7)!=0);//数据包号
static const uint16_t s21c_RecvDoneMask = (1<<s21c_packetNum)-1;                   //数据解析完成标志位
static uint8_t s21c_buffer[s21c_packetLen];                                          //存放解析数据的缓冲区
static uint16_t s21c_RecvRealState = 0;
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if( RxMessage.IDE==CAN_Id_Standard )
	{
		switch( RxMessage.StdId ) //帧ID号
		{
			...
			{
				//标记已处理的包号
				s21c_RecvRealState |= 1<<temp_rxbuf[0];
				
				//根据包号将数据存放到对应的数据缓冲区
				memcpy(s21c_buffer+temp_rxbuf[0]*7,temp_rxbuf+1,RxMessage.DLC-1);
				
				//数据解析完成后进行解析(仅在最后一个包号时检查是否接收完成,避免出现数据错位)
				if( (temp_rxbuf[0] == s21c_packetNum-1) && ((s21c_RecvRealState&s21c_RecvDoneMask)==s21c_RecvDoneMask) )
				{
					s21c_RecvRealState=0;//清空标志位等待下一轮解析
					memcpy((uint8_t*)&s21c_board,s21c_buffer,s21c_packetLen);	
					SysVal.HardWare_Ranger = 1;
				}
