...
- **红外测距×3 数据不上行**：固件没有红外测距采集代码，24 字节帧也无此字段。**已定方案（2026-07-26）**：红外为 PWM 输出，接 STM32 定时器输入捕获（非 ADC），固件扩展上行帧带 3 路距离（uint16 mm）→ K7 串口节点解析后发布 3 路 `sensor_msgs/Range`（`/ir_sensor/{front,left45,right45}`）。PWM 引脚/定时器通道与脉宽-距离换算公式待避障组确认。
