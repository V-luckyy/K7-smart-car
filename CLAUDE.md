...
└── sensor_data\                                ← 传感器资料
    └── GY-53 VL53L0X\                          ← 红外测距模块 ×3（VL53L0X ToF）
        ├── GY-53使用说明v1.pdf                    ← 手册（PWM/UART 协议）
        ├── STM32/  (F103 参考工程 PWM/USART, .gitignore 已排除)
        └── pc软件/  (上位机, .gitignore 已排除)
```
...
- [ ] **是否需要其他传感器**（超声波、ToF、激光雷达？）— 注：C50X 板载红外测距×3，已定 PWM 输入捕获 + 固件扩展上行帧方案（2026-07-26，用于番外线 MPC 避障），见第 13.3 节
...
- **红外测距×3（GY-53 / VL53L0X ToF，资料在 `sensor_data/GY-53 VL53L0X/`）**：固件当前无采集代码，24 字节帧无此字段。**已定方案（2026-07-26）**：GY-53 PWM 输出（20Hz 方波，**距离 mm = 高电平 us ÷ 10**）接 STM32 定时器输入捕获（非 ADC），固件扩展上行帧带 3 路距离（uint16 mm）→ K7 串口节点解析发布 3 路 `sensor_msgs/Range`。量程/更新率随测量模式：高精度 0~1.2m / 200ms / ±1cm（默认）、一般 0~1.2m / 35ms / ±2cm、快速 0~1.2m / 22ms / ±3cm、长距离 0~2m / 35ms / ±4cm（串口命令可切、掉电保存，建议快速或一般模式）。PWM 引脚/定时器通道与安装角度待避障组确认。
